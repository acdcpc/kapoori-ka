...
export const getMilestonesForAge = (ageMonths: number): Milestone[] => {
  ...
  // Show from 2 bands behind current up to 1 band ahead
  const showFrom = currentBandIndex >= 2 ? AGE_BANDS[currentBandIndex - 2] : AGE_BANDS[0];
  const showUpTo = currentBandIndex < AGE_BANDS.length - 1
    ? AGE_BANDS[currentBandIndex + 1]
    : AGE_BANDS[AGE_BANDS.length - 1];

  return MILESTONES.filter(m => m.ageMonthsMax >= showFrom && m.ageMonthsMax <= showUpTo);
};