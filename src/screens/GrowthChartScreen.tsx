...
// ─────────────────────────────────────────────────────────────────
// Extended WHO Weight-for-Age data: 0-216 months (0-18 years)
// Format: [ageMonths, SD3neg, SD2neg, Median, SD2pos, SD3pos]
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Extended WHO Height-for-Age data: 0-216 months (0-18 years)
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// WHO BMI-for-Age data: 24-216 months (2-18 years)
// Format: [ageMonths, SD3neg, SD2neg, Median, SD1pos, SD2pos]
// ─────────────────────────────────────────────────────────────────
...
function getIdealRanges(ageMonths: number, sex: 'male' | 'female', metric: 'weight' | 'height' | 'bmi') {
  let curves: number[][];
  if (metric === 'weight') {
    curves = sex === 'male' ? WHO_WFA_BOYS : WHO_WFA_GIRLS;
  } else if (metric === 'height') {
    curves = sex === 'male' ? WHO_HFA_BOYS : WHO_HFA_GIRLS;
  } else {
    curves = sex === 'male' ? WHO_BFA_BOYS : WHO_BFA_GIRLS;
  }
...
function getNutritionalStatus(val: number, ranges: any, isNe: boolean, metric: 'weight' | 'height' | 'bmi'): StatusResult | null {
  ...
  const sd2neg = ranges.sd2neg;
  const sd2pos = ranges.sd2pos;
  const sd3pos = ranges.highest;

  if (metric === 'bmi') {
    if (val < sd3neg) {
      return {
        label: 'Severely Wasted', labelNe: 'अति कम BMI',
        color: '#B71C1C', category: 'Severely Wasted', categoryNe: 'अति कम BMI (गम्भीर)',
        description: 'Your child has severe acute malnutrition based on BMI-for-age (below -3 SD). Immediate medical attention needed.',
        descriptionNe: 'BMI आधारमा तपाईंको बच्चामा गम्भीर कुपोषण छ। तुरुन्त चिकित्सकीय ध्यान आवश्यक।',
        needsDoctor: true,
      };
    } else if (val < sd2neg) {
      return {
        label: 'Wasted', labelNe: 'कम BMI',
        color: '#F44336', category: 'Wasted', categoryNe: 'कम BMI',
        description: 'Your child is wasted (below -2 SD BMI-for-age). Indicates acute malnutrition.',
        descriptionNe: 'BMI आधारमा कम छ (−२ SD भन्दा कम)। यो तीव्र कुपोषणको संकेत हो।',
        needsDoctor: true,
      };
    } else if (val > sd2pos) {
      return {
        ...
        description: 'Your child is obese based on BMI-for-age (above +2 SD). Lifestyle intervention recommended.',
        descriptionNe: 'BMI आधारमा मोटोपना छ (+२ SD भन्दा बढी)। जीवनशैली परिवर्तन सिफारिस गरिन्छ।',
        needsDoctor: true,
      };
    } else if (val > ranges.ideal + (ranges.sd2pos - ranges.ideal) * 0.5) {
      return {
        label: 'Overweight', labelNe: 'बढी BMI',
        color: '#FF9800', category: 'Overweight', categoryNe: 'बढी BMI',
        description: 'Your child is overweight. Monitor diet and increase physical activity.',
        descriptionNe: 'BMI सामान्यभन्दा बढी छ। खाना र शारीरिक गतिविधिमा ध्यान दिनुहोस्।',
        needsDoctor: false,
      };
    } else {
      return {
        ...
        description: 'Your child\'s BMI is normal for their age. Keep up the healthy habits!',
        descriptionNe: 'BMI उमेर अनुसार सामान्य छ। स्वस्थ बानी कायम राख्नुहोस्!',
        needsDoctor: false,
      };
    }
  }
...
export default function GrowthChartScreen({ route, navigation }: Props) {
  ...
  const saveRecord = async () => {
    ...
    try {
      const user = auth.currentUser;
      const ageMonths = getAgeInMonths(child.dateOfBirth, todayAd);
  ...
  const chartData = useMemo(() => {
    if (chartType === 'bmi') {
      return records
        .filter(r => r.weight && r.height && (r.ageMonths || getAgeInMonths(child.dateOfBirth, r.date)) >= 24)
        .map(r => {
          const age = r.ageMonths || getAgeInMonths(child.dateOfBirth, r.date);
          return { x: age, y: calculateBMI(r.weight, r.height || 0) };
        })
        .filter(d => d.y > 0);
    }
    return records.map(r => ({ 
      x: r.ageMonths || getAgeInMonths(child.dateOfBirth, r.date), 
      y: (chartType === 'weight' ? r.weight : r.height) || 0 
    })).filter(d => d.y > 0);
  }, [records, chartType, child.dateOfBirth]);

  const latestRecord = records.length > 0 ? records[records.length - 1] : null;
  
  // For BMI, use latest record with both weight and height, age >= 24 months
  const latestBMIRecord = useMemo(() => {
    for (let i = records.length - 1; i >= 0; i--) {
      const r = records[i];
      const age = r.ageMonths || getAgeInMonths(child.dateOfBirth, r.date);
      if (r.weight && r.height && age >= 24) {
        return { ...r, ageMonths: age, bmi: calculateBMI(r.weight, r.height) };
      }
    }
    return null;
  }, [records, child.dateOfBirth]);

  const currentVal = chartType === 'weight' 
    ? latestRecord?.weight 
    : chartType === 'height' 
      ? latestRecord?.height 
      : latestBMIRecord?.bmi;

  const displayAgeMonths = chartType === 'bmi' && latestBMIRecord
    ? latestBMIRecord.ageMonths
    : (latestRecord ? (latestRecord.ageMonths || getAgeInMonths(child.dateOfBirth, latestRecord.date)) : 0);
  ...
  // WHO curves based on chart type
  ...
  // BMI available only for age >= 24 months
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{paddingBottom: 40}}>
      {/* Tab Switcher: Chart vs Predictor */}
      <View style={styles.tabContainer}>
        <TouchableOpacity 
          style={[styles.tab, activeTab === 'chart' && styles.tabActive]} 
          onPress={() => setActiveTab('chart')}
        >
          <Ionicons name="analytics" size={18} color={activeTab === 'chart' ? '#1a73e8' : '#666'} />
          <Text style={[styles.tabText, activeTab === 'chart' && styles.tabTextActive]}>
            {isNe ? 'वृद्धि चार्ट' : 'Growth Chart'}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity 
          style={[styles.tab, activeTab === 'predictor' && styles.tabActive]} 
          onPress={() => setActiveTab('predictor')}
        >
          <Ionicons name="trending-up" size={18} color={activeTab === 'predictor' ? '#1a73e8' : '#666'} />
          <Text style={[styles.tabText, activeTab === 'predictor' && styles.tabTextActive]}>
            {isNe ? 'भविष्य उचाइ' : 'Height Predictor'}
      ...
      {activeTab === 'chart' ? (
        <>
          ...
          <View style={styles.disclaimerBanner}>
            <Text style={styles.disclaimerText}>
              {isNe 
                ? '⚠️ यो शैक्षिक सन्दर्भ मात्र हो। चिकित्सकीय सल्लाहको विकल्प होइन।'
            ...
            <View style={[styles.statusCard, { borderLeftColor: status.color }]}>
              <View style={styles.statusHeader}>
                <Text style={styles.statusTitle}>
                  {chartType === 'bmi' 
                    ? (isNe ? 'BMI स्थिति' : 'BMI Status')
                    : (isNe ? 'वृद्धि स्थिति' : 'Growth Status')}: 
                </Text>
              ...
              <Text style={styles.statusDesc}>
                {isNe ? status.descriptionNe : status.description}
              </Text>
              {status.needsDoctor && (
                <View style={styles.alertBox}>
                  ...
                  <Text style={styles.alertText}>
                    {isNe
                      ? 'तपाईंको बच्चालाई थप मूल्यांकनको लागि बाल रोग विशेषज्ञसँग परामर्श लिनुहोस्।'
                      : 'Please have your child evaluated by a pediatrician for further assessment.'}
                  </Text>
                </View>
              )}
              {status.label !== 'Normal' && chartType !== 'bmi' && (
                <TouchableOpacity
                ...
                >
                  ...
                  <Text style={styles.nutritionLinkText}>
                    {isNe ? 'पोषण सल्लाह हेर्नुहोस्' : 'View Nutrition & Feeding Advice'}
                  </Text>
          ...
          <View style={styles.currentInfoCard}>
            <Text style={styles.currentInfoTitle}>
              {isNe ? 'हालको मापन' : 'Current Measurement'}
            </Text>
            <View style={styles.currentInfoRow}>
              <View style={styles.currentInfoItem}>
                <Text style={styles.currentInfoLabel}>{isNe ? 'तौल' : 'Weight'}</Text>
                <Text style={styles.currentInfoValue}>
                  {latestRecord?.weight ? `${latestRecord.weight} kg` : (isNe ? 'उपलब्ध छैन' : 'N/A')}
                </Text>
              </View>
              <View style={styles.currentInfoItem}>
                <Text style={styles.currentInfoLabel}>{isNe ? 'उचाइ' : 'Height'}</Text>
                <Text style={styles.currentInfoValue}>
                  {latestRecord?.height ? `${latestRecord.height} cm` : (isNe ? 'उपलब्ध छैन' : 'N/A')}
                </Text>
              </View>
              <View style={styles.currentInfoItem}>
                <Text style={styles.currentInfoLabel}>{isNe ? 'उमेर' : 'Age'}</Text>
                <Text style={styles.currentInfoValue}>
                  {childAgeMonths} {isNe ? 'महिना' : 'mo'}
                </Text>
              </View>
            </View>
            {latestBMIRecord && (
              <View style={styles.bmiRow}>
                ...
                <Text style={styles.bmiValue}>{latestBMIRecord.bmi.toFixed(1)}</Text>
                <Text style={styles.bmiAge}>({isNe ? 'उमेर ' : 'Age '}{latestBMIRecord.ageMonths} {isNe ? 'महिना' : 'mo'})</Text>
              </View>
            )}
            {ranges && (
              <View style={styles.idealRangeBox}>
                <Text style={styles.idealRangeTitle}>
                  {isNe ? 'WHO मापदण्ड (उमेर ' : 'WHO Standards (Age '}{ranges.age}{isNe ? ' महिना)' : ' months)'}
                </Text>
                <View style={styles.idealRangeRow}>
                  <View style={styles.idealRangeItem}>
                    <Text style={styles.idealRangeLabel}>{isNe ? 'न्यूनतम' : 'Lowest'}</Text>
                    <Text style={styles.idealRangeValue}>{ranges.lowest} {chartType === 'bmi' ? '' : (chartType === 'weight' ? 'kg' : 'cm')}</Text>
                  </View>
                  <View style={styles.idealRangeItem}>
                    <Text style={styles.idealRangeLabel}>{isNe ? 'सामान्य' : 'Normal'}</Text>
                    <Text style={styles.idealRangeValue}>{ranges.ideal} {chartType === 'bmi' ? '' : (chartType === 'weight' ? 'kg' : 'cm')}</Text>
                  </View>
                  <View style={styles.idealRangeItem}>
                    <Text style={styles.idealRangeLabel}>{isNe ? 'अधिकतम' : 'Highest'}</Text>
                    <Text style={styles.idealRangeValue}>{ranges.highest} {chartType === 'bmi' ? '' : (chartType === 'weight' ? 'kg' : 'cm')}</Text>
                  </View>
                </View>
              </View>
            )}
          ...
          <View style={styles.toggleContainer}>
            ...
            >
              <Text style={[styles.toggleText, chartType === 'weight' && styles.toggleTextActive]}>
                {isNe ? 'तौल' : 'Weight'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity 
            ...
            >
              <Text style={[styles.toggleText, chartType === 'height' && styles.toggleTextActive]}>
                {isNe ? 'उचाइ' : 'Height'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity 
              style={[styles.toggleBtn, chartType === 'bmi' && styles.toggleBtnActive, !bmiAvailable && styles.toggleBtnDisabled]} 
              onPress={() => bmiAvailable && setChartType('bmi')}
              disabled={!bmiAvailable}
            >
              <Text style={[styles.toggleText, chartType === 'bmi' && styles.toggleTextActive, !bmiAvailable && styles.toggleTextDisabled]}>
                BMI {bmiAvailable ? '' : '(2y+)'}
              </Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.chartWrapper}>
            <Text style={styles.chartTitle}>
              {chartType === 'weight' 
                ? (isNe ? 'तौल वृद्धि चार्ट (WHO)' : 'Weight Growth Chart (WHO)')
                : chartType === 'height'
                  ? (isNe ? 'उचाइ वृद्धि चार्ट (WHO)' : 'Height Growth Chart (WHO)')
                  : (isNe ? 'BMI चार्ट (WHO)' : 'BMI-for-Age Chart (WHO)')}
            </Text>
            <VictoryChart 
              width={CHART_WIDTH} 
              height={CHART_HEIGHT} 
            ...
            >
              <VictoryAxis 
                label={isNe ? 'उमेर (महिना)' : 'Age (months)'} 
              ...
              <VictoryAxis 
                ...
                label={`${
                  chartType === 'weight' 
                    ? (isNe ? 'तौल (केजी)' : 'Weight (kg)') 
                    : chartType === 'height'
                      ? (isNe ? 'उचाइ (सेमी)' : 'Height (cm)')
                      : 'BMI (kg/m²)'
                }`} 
                style={{ axisLabel: { padding: 40, fontSize: 10 } }} 
              />
              ...
              <VictoryLine data={med} style={{ data: { stroke: '#4CAF50', strokeWidth: 1.5, strokeDasharray: '4,4' } }} />
              <VictoryLine data={sd2n} style={{ data: { stroke: '#FF9800', strokeWidth: 1, opacity: 0.6 } }} />
              <VictoryLine data={sd2p} style={{ data: { stroke: '#FF9800', strokeWidth: 1, opacity: 0.6 } }} />
            ...
            <View style={styles.legendRow}>
              <View style={styles.legendItem}><View style={[styles.legendDot, {backgroundColor:'#1a73e8'}]} /><Text style={styles.legendText}>{isNe ? 'तपाईंको बच्चा' : 'Your Child'}</Text></View>
              <View style={styles.legendItem}><View style={[styles.legendDot, {backgroundColor:'#4CAF50'}]} /><Text style={styles.legendText}>{isNe ? 'WHO मध्यम' : 'WHO Median'}</Text></View>
              <View style={styles.legendItem}><View style={[styles.legendDot, {backgroundColor:'#FF9800'}]} /><Text style={styles.legendText}>{isNe ? '±2 SD' : '±2 SD'}</Text></View>
            </View>
          </View>

          {/* Add Button - Premium Guard */}
          {isPremium ? (
            <TouchableOpacity style={styles.addBtn} onPress={() => setShowForm(true)}>
              <Ionicons name="add-circle" size={24} color="#fff" />
              <Text style={styles.addBtnText}>
                {isNe ? 'नयाँ मापन थप्नुहोस्' : 'Add New Measurement'}
              </Text>
            </TouchableOpacity>
          ) : (
            <TouchableOpacity style={styles.addBtnLocked} onPress={() => navigation.navigate('Subscription' as any)}>
              ...
              <Text style={styles.addBtnText}>
                {isNe ? 'नयाँ मापन थप्न प्रिमियम अपग्रेड गर्नुहोस्' : 'Upgrade to Premium to Add Measurements'}
              </Text>
            </TouchableOpacity>
          )}
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                ...
                <Text style={styles.modalSubtitle}>
                  {isNe ? 'मिति: ' : 'Date: '}{todayAd}
                </Text>
                <TextInput 
                  style={styles.input} 
                  placeholder={isNe ? 'तौल (केजी)' : 'Weight (kg)'} 
                ...
                <TextInput 
                  style={styles.input} 
                  placeholder={isNe ? 'उचाइ (सेमी) - ऐच्छिक' : 'Height (cm) - optional'} 
                  keyboardType="numeric" 
                  value={height} 
                ...
                <View style={styles.modalBtns}>
                  ...
                  <TouchableOpacity style={styles.saveBtn} onPress={saveRecord} disabled={saving}>
                    <Text style={styles.saveBtnText}>{saving ? '...' : (isNe ? 'बचत' : 'Save')}</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </View>
        ...
        /* Height Predictor Tab - Premium Guard */
        <PremiumGuard>
          <View style={styles.predictorContainer}>
            <View style={styles.predictorCard}>
              <Ionicons name="trending-up" size={40} color="#1a73e8" style={{ alignSelf: 'center', marginBottom: 12 }} />
              <Text style={styles.predictorTitle}>
                {isNe ? 'बच्चाको सम्भावित वयस्क उचाइ' : 'Predict Your Child\'s Likely Adult Height'}
              </Text>
              <Text style={styles.predictorSubtitle}>
                {isNe 
                  ? 'आमाबुबाको उचाइ आधारमा बच्चाको सम्भावित वयस्क उचाइ अनुमान गर्नुहोस् (Mid-parental height)' 
                  : 'Estimate your child\'s adult height based on parents\' heights (Mid-parental height)'}
              </Text>
              
              <View style={styles.predictorInputRow}>
                <View style={styles.predictorInputGroup}>
                  <Text style={styles.predictorLabel}>
                    {isNe ? 'बुबाको उचाइ (सेमी)' : 'Father\'s Height (cm)'}
                  </Text>
                  <TextInput
                ...
                <View style={styles.predictorInputGroup}>
                  <Text style={styles.predictorLabel}>
                    {isNe ? 'आमाको उचाइ (सेमी)' : 'Mother\'s Height (cm)'}
                  </Text>
                  <TextInput
                    style={styles.predictorInput}
                    keyboardType="numeric"
              ...
              <TouchableOpacity 
                style={styles.predictorBtn} 
                onPress={() => setShowPrediction(true)}
              >
                <Text style={styles.predictorBtnText}>
                  {isNe ? 'अनुमान गर्नुहोस्' : 'Calculate'}
                </Text>
              </TouchableOpacity>
              ...
              {showPrediction && predictedHeight && (
                <View style={styles.predictionResult}>
                  <Text style={styles.predictionLabel}>
                    {isNe ? 'सम्भावित वयस्क उचाइ:' : 'Predicted Adult Height:'}
                  </Text>
                  ...
                  <Text style={styles.predictionRange}>
                    {isNe 
                      ? `(सामान्य दायरा: ${(predictedHeight - 8.5).toFixed(1)} - ${(predictedHeight + 8.5).toFixed(1)} cm)`
                      : `(Normal range: ${(predictedHeight - 8.5).toFixed(1)} - ${(predictedHeight + 8.5).toFixed(1)} cm)`}
                  </Text>
                  <Text style={styles.predictionNote}>
                    {isNe
                      ? 'यो अनुमान मात्र हो। अन्तिम उचाइ जीन, पोषण, र स्वास्थ्यमा भर पर्छ।'
                      : 'This is an estimate only. Final height depends on genetics, nutrition, and overall health.'}
                  </Text>
                </View>
              )}
            </View>
            ...
            <View style={styles.predictorInfoCard}>
              <Text style={styles.predictorInfoTitle}>
                {isNe ? 'के हो Mid-parental Height?' : 'What is Mid-parental Height?'}
              </Text>
              <Text style={styles.predictorInfoText}>
                {isNe
                  ? 'Mid-parental height एक बालरोग उपकरण हो जसले आमाबुबाको उचाइ आधारमा बच्चाको सम्भावित वयस्क उचाइ अनुमान गर्छ।\n\nसूत्र:\nछोराको लागि: (बुबाको उचाइ + आमाको उचाइ + १३ सेमी) ÷ २\nछोरीको लागि: (बुबाको उचाइ + आमाको उचाइ - १३ सेमी) ÷ २\n\nयो अनुमान मात्र हो र ±८.५ सेमीको दायरामा हुनसक्छ।'
                  : 'Mid-parental height is a pediatric tool used to estimate a child\'s likely adult height based on the biological parents\' heights.\n\nFormula:\nFor boys: (Father\'s height + Mother\'s height + 13 cm) ÷ 2\nFor girls: (Father\'s height + Mother\'s height - 13 cm) ÷ 2\n\nThis is only an estimate and can vary by ±8.5 cm.'}
              </Text>
            </View>
          </View>
        </PremiumGuard>
...
const styles = StyleSheet.create({
  ...
  // Tabs
  tabContainer: { flexDirection: 'row', marginHorizontal: 12, marginTop: 12, marginBottom: 8, backgroundColor: '#e0e0e0', borderRadius: 10, padding: 4 },
  ...
  premiumBadge: { color: '#FF9800', fontSize: 12 },

  // Disclaimer
  disclaimerBanner: { backgroundColor: '#fff3e0', padding: 10, marginHorizontal: 12, marginBottom: 8, borderRadius: 8, borderLeftWidth: 4, borderLeftColor: '#ff9800' },
  disclaimerText: { fontSize: 12, color: '#e65100', lineHeight: 18 },

  // Status Card
  statusCard: { backgroundColor: '#fff', marginHorizontal: 12, marginBottom: 12, borderRadius: 12, padding: 16, elevation: 2, borderLeftWidth: 5 },
  ...
  // Current Info Card
  currentInfoCard: { backgroundColor: '#fff', marginHorizontal: 12, marginBottom: 12, borderRadius: 12, padding: 16, elevation: 2 },
  currentInfoTitle: { fontSize: 16, fontWeight: '700', marginBottom: 12, color: '#333' },
  currentInfoRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 },
  currentInfoItem: { alignItems: 'center', flex: 1 },
  currentInfoLabel: { fontSize: 12, color: '#888', marginBottom: 4 },
  currentInfoValue: { fontSize: 18, fontWeight: 'bold', color: '#1a73e8' },
  bmiRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 12, backgroundColor: '#E8F5E9', padding: 8, borderRadius: 8 },
  bmiLabel: { fontSize: 14, fontWeight: '600', color: '#2E7D32' },
  bmiValue: { fontSize: 18, fontWeight: 'bold', color: '#2E7D32', marginLeft: 8 },
  bmiAge: { fontSize: 12, color: '#666', marginLeft: 8 },
  idealRangeBox: { backgroundColor: '#f5f5f5', borderRadius: 8, padding: 12, marginTop: 4 },
  idealRangeTitle: { fontSize: 13, fontWeight: '600', color: '#555', marginBottom: 8 },
  idealRangeRow: { flexDirection: 'row', justifyContent: 'space-between' },
  idealRangeItem: { alignItems: 'center', flex: 1 },
  idealRangeLabel: { fontSize: 11, color: '#888', marginBottom: 2 },
  idealRangeValue: { fontSize: 14, fontWeight: '700', color: '#333' },

  // Toggle
  toggleContainer: { flexDirection: 'row', marginHorizontal: 12, marginBottom: 12, backgroundColor: '#eee', borderRadius: 8, padding: 4 },
  ...
  toggleTextDisabled: { color: '#aaa' },

  // Chart
  chartWrapper: { backgroundColor: '#fff', marginHorizontal: 12, borderRadius: 12, padding: 8, elevation: 2, marginBottom: 12 },
  chartTitle: { fontSize: 14, fontWeight: '600', color: '#666', marginBottom: 10, textAlign: 'center' },
  legendRow: { flexDirection: 'row', justifyContent: 'center', gap: 16, marginTop: 8, marginBottom: 4 },
  legendItem: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  legendDot: { width: 10, height: 10, borderRadius: 5 },
  legendText: { fontSize: 11, color: '#666' },

  // Add Button
  addBtn: { backgroundColor: '#1a73e8', marginHorizontal: 12, padding: 16, borderRadius: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', elevation: 3, marginBottom: 12 },
  addBtnLocked: { backgroundColor: '#FF9800', marginHorizontal: 12, padding: 16, borderRadius: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', elevation: 3, marginBottom: 12 },
  ...
  modalSubtitle: { fontSize: 13, color: '#888', marginBottom: 16, textAlign: 'center' },
  ...
  predictorCard: { backgroundColor: '#fff', borderRadius: 16, padding: 20, elevation: 2, marginBottom: 12 },
  predictorTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', textAlign: 'center', marginBottom: 8 },
  predictorSubtitle: { fontSize: 13, color: '#666', textAlign: 'center', marginBottom: 20, lineHeight: 18 },
  predictorInputRow: { flexDirection: 'row', gap: 12, marginBottom: 16 },
  predictorInputGroup: { flex: 1 },
  predictorLabel: { fontSize: 13, color: '#666', marginBottom: 6 },
  predictorInput: { borderWidth: 1, borderColor: '#ddd', borderRadius: 10, padding: 12, fontSize: 16, backgroundColor: '#fcfcfc' },
  predictorBtn: { backgroundColor: '#1a73e8', borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 4 },
  predictorBtnText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  predictionResult: { backgroundColor: '#E8F5E9', borderRadius: 12, padding: 16, marginTop: 16, alignItems: 'center' },
  predictionLabel: { fontSize: 14, color: '#555', marginBottom: 4 },
  predictionValue: { fontSize: 32, fontWeight: 'bold', color: '#2E7D32' },
  predictionRange: { fontSize: 13, color: '#666', marginTop: 4 },
  predictionNote: { fontSize: 12, color: '#888', marginTop: 8, textAlign: 'center', fontStyle: 'italic' },
  predictorInfoCard: { backgroundColor: '#E3F2FD', borderRadius: 16, padding: 20, marginBottom: 20 },
  predictorInfoTitle: { fontSize: 16, fontWeight: '700', color: '#1565C0', marginBottom: 10 },
  predictorInfoText: { fontSize: 13, color: '#444', lineHeight: 20 },
});